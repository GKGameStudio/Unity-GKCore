...
namespace GKCore.Observers.Editors
{
    ...
    public class DocumentationEditor : Editor
    {
        ...
        {
            GUILayout.Label("Value Observer", CenteredLabelStyle);
            GUILayout.Label("by Infinity Code", CenteredLabelStyle);
            GUILayout.Label("version: " + EditorUtils.Version, CenteredLabelStyle);
            GUILayout.Space(10);
        }

        private void DrawRateAndReview()
        {
            EditorGUILayout.HelpBox("Please don't forget to leave a review on the store page if you liked Value Observer, this helps us a lot!", MessageType.Warning);
